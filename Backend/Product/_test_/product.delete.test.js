...
describe('DELETE /api/products/:id', () => {
    ...
    it('returns 400 for invalid id format', async () => {
        ...
        const res = await request(app).delete('/api/products/123-sample-id').set('Authorization', `Bearer ${token}`);
    ...
    it('returns 404 when product not found', async () => {
        ...
        const res = await request(app).delete(`/api/products/${fakeId}`).set('Authorization', `Bearer ${token}`);
