...
module.exports = function () {
   ...
   subscribeToQueue('ORDER_SELLER_DASHBOARD.ORDER_UPDATED', async (data) => {
    try {
        ...
        if (!updated) {
            console.log(`ORDER_UPDATED: no matching order found for ${data.orderId}`);
        } else {
            console.log(`ORDER_UPDATED: order ${data.orderId} set to ${data.status}`);
        }
    } catch (err) {
        console.log(`ORDER_UPDATED failed for ${data.orderId}:`, err.message);
    }
});
